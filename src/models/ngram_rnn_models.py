...
data_path = os.path.join(data_dir, 'generated_data', 'sequence_data.json')
...
def train_rnn_model(training_data, embedding_dim=32, lstm_units=64, max_sequence_length=10, epochs=30):
    ...
    model.fit(X_padded, y_onehot, epochs=epochs, batch_size=32,
...
def main():
    ...
    parser.add_argument('--mode', choices=['train', 'eval', 'predict', 'all'], default='train', help="Mode to run (default: train; 'all' runs train, eval, and predict)")
    ...
    if args.model == 'ngram':
        ...
        if args.mode == 'all':
            print(f"\nRunning ALL modes for N-gram model...\n")
        ...
        if args.mode == 'all':
            print(f"\nRunning ALL modes for RNN model...\n")
