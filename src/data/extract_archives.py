...
#
# The full provided by the authors dataset can be found here: https://github.com/congy/AutoSuggest
#
...
def get_table_hash(dataframe):
    """
    Generates a hash for a dataframe to identify duplicate tables.
...
def extract_samples():
    """
    Extracts unique samples from archives for each operator.
            ...
            for idx, (sample_dir, csv_files) in enumerate(valid_samples, 1):
                ...
                if operator == "merge":
                    ...
                    if len(csv_files) >= 2:
                        ...
                        csv_files.sort(key=lambda csvfile: os.path.getsize(os.path.join(sample_output_dir, csvfile)))
