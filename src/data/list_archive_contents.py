...
def list_archive_contents(archive_path):
    """
    Lists key contents of a tar archive without showing all members.
    """
...
for arch_path in tgz_files:
    # Get file size in MB
    size_mb = os.path.getsize(arch_path) / (1024 * 1024)
    print(f"\n{'=' * 50}")
    print(f"Archive: {os.path.basename(arch_path)}")
    print(f"Size: {size_mb:.2f} MB")

    list_archive_contents(arch_path)
