...
def main():
    ...
    parser.add_argument('--data_dir', default='data/extracted_data',
                        help='Directory containing the extracted data samples') # e.g. python -m src.main --results_dir output_results/
    parser.add_argument('--models_dir', default='models',
                        help='Directory to save trained models')    # e.g. python -m src.main --models_dir saved_models/
    parser.add_argument('--results_dir', default='results',
                        help='Directory for results and figures')   # e.g. python -m src.main --results_dir output_results/
    parser.add_argument('--operator', choices=['join', 'groupby', 'pivot', 'unpivot', 'all'],
                        default='all', help='Which operator to run (default: all)')
    ...
    parser.add_argument('--top_k', type=int, default=2,
                        help='Number of top recommendations to display in prediction mode') # e.g. python -m src.main --operator groupby --mode predict --top_k 3
    parser.add_argument('--aggfunc', type=str, default='mean',
                        help='Aggregation function for pivot (default: mean)')  # e.g. python -m src.main --operator pivot --mode predict --input_file data/pivot/sample.csv --aggfunc sum
    ...
    if args.operator in ['groupby', 'all']:
        try:
            # Call the groupby pipeline with all necessary args
            success = run_groupby_prediction(
                args.data_dir, models_dir, mode=args.mode, input_file=args.input_file, top_k=top_k)
            ...
            print(f"Error during GroupBy prediction: {e}")
            traceback.print_exc()
            if args.operator == 'groupby':
                return
    ...
    # Print summary if all operators were run in train mode
    if args.operator == 'all' and args.mode == 'train':
        print("\n" + "=" * 80)
        print("Summary of Results")
        print("=" * 80)

        print("Join column prediction: Success")
        print("GroupBy column prediction: Success")

        if 'pivot' in results:
            print("Pivot prediction (AMPT): Success")
            # for metric, value in results['pivot'].items():
            #     if isinstance(value, float):
            #         print(f"  {metric}: {value:.4f}")
            #     else:
            #         print(f"  {metric}: {value}")

        if 'unpivot' in results:
            print("Unpivot prediction (CMUT): Success")
            # for metric, value in results['unpivot'].items():
            #     if isinstance(value, float):
            #         print(f"  {metric}: {value:.4f}")
            #     else:
            #         print(f"  {metric}: {value}")
